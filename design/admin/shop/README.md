...
- Product detail panel now has an internal scroll on desktop when the description is long.
- WYSIWYG description content can scroll inside its block and can still be expanded.
- Product gallery no longer stretches thumbnails when the right product information panel is taller.
- Main product image and thumbnails have stable desktop, tablet, and mobile sizes.


Part of `Minimal Shop UI Kit Final`. See `../design-guidelines/index.html` for shared design guidelines.
